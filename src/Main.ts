...
export class Main extends LitElement {
  ...
  protected render(): TemplateResult | void {
    ...
    return html`
        <div class="sidebar">
          <div class="menu">
            <paper-icon-item tabindex="0" role="option">
              <iron-icon class="iconify" icon="menu" @click=${this.expandSidebar}></iron-icon>
        ...
        <app-header  slot="header" effects="waterfall" fixed="" condenses="">
          ...
          <div sticky id="sticky">
            ...
            ${this.docs[this.category!].map(element => {
              return html`
                <paper-tab page-name="${element.title}">${element.title.toUpperCase()}</paper-tab>
              `;
            })}
            </paper-tabs>
          </div>
  ...
  resizeSidebarSpacer() {
    ...
    space += this.getHeightInt(main!.querySelector('.menu'));
    (main!.querySelector('.spacer') as HTMLElement).style.height = `calc(100vh - ${space + 136}px)`;
  }

  getHeightInt(element) {
    return parseInt(
      window
